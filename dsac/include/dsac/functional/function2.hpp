...
#if !defined(FU2_WITH_NO_FUNCTIONAL_HEADER) && !defined(FU2_NO_FUNCTIONAL_HEADER) && \
...
constexpr auto invoke(Type T::*member, Self&& self) noexcept(noexcept(std::forward<Self>(self).*member))
...
struct can_invoke<T, identity<Args...>, decltype((void)std::declval<T>()(std::declval<Args>()...))> : std::true_type {
};
template <typename Pointer, typename T, typename... Args>
struct can_invoke<
    Pointer,
    identity<T&, Args...>,
    decltype((void)((std::declval<T&>().*std::declval<Pointer>())(std::declval<Args>()...)))> : std::true_type {
};
template <typename Pointer, typename T, typename... Args>
struct can_invoke<
    Pointer,
...
struct can_invoke<Pointer, identity<T&>, decltype((void)(std::declval<T&>().*std::declval<Pointer>()))>
  : std::true_type {
};
template <typename Pointer, typename T>
struct can_invoke<Pointer, identity<T&&>, decltype((void)(std::declval<T&&>().*std::declval<Pointer>()))>
...
struct is_noexcept_correct<true, T, identity<Args...>>
  : std::integral_constant<
        bool,
        noexcept(::fu2::detail::invocation::invoke(std::declval<T>(), std::declval<Args>()...))> {
};
...
struct overload_impl<Current, Next, Rest...> : Current, overload_impl<Next, Rest...> {
...
struct overload_impl<Current> : Current {
  explicit overload_impl(Current current)
    : Current(std::move(current))
  {
  }
...
union data_accessor {
  ...
  explicit constexpr data_accessor(void* ptr) noexcept
    : ptr_(ptr)
  {
  }
...
using transfer_const_t =
    std::conditional_t<std::is_const<std::remove_pointer_t<From>>::value, std::add_const_t<To>, To>;
template <typename From, typename To>
using transfer_volatile_t =
    std::conditional_t<std::is_volatile<std::remove_pointer_t<From>>::value, std::add_volatile_t<To>, To>;

/// The retriever when the object is allocated inplace
template <typename T, typename Accessor>
FU2_DETAIL_CXX14_CONSTEXPR auto retrieve(std::true_type /*is_inplace*/, Accessor from, std::size_t from_capacity)
{
...
[[noreturn]] void throw_or_abortnoexcept(std::integral_constant<bool, IsNoexcept> /*is_throwing*/) noexcept
{
  std::abort();
}
/// Calls std::abort on empty function calls
[[noreturn]] inline void throw_or_abort(std::false_type /*is_throwing*/) noexcept
{
  std::abort();
}
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                                           \
  ...
  struct function_trait<Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT> {                                              \
    using pointer_type = Ret (*)(data_accessor CONST VOLATILE*, std::size_t capacity, Args...);                      \
    template <typename T, bool IsInplace>                                                                            \
    struct internal_invoker {                                                                                        \
      static Ret invoke(data_accessor CONST VOLATILE* data, std::size_t capacity, Args... args) NOEXCEPT             \
      {                                                                                                              \
        auto obj = retrieve<T>(std::integral_constant<bool, IsInplace>{}, data, capacity);                           \
    ...
    template <typename T>                                                                                            \
    struct view_invoker {                                                                                            \
      static Ret invoke(data_accessor CONST VOLATILE* data, std::size_t, Args... args) NOEXCEPT                      \
      {                                                                                                              \
        auto ptr = static_cast<void CONST VOLATILE*>(data->ptr_);                                                    \
        return invocation::invoke(address_taker<T>::restore(ptr), std::forward<Args>(args)...);                      \
      }                                                                                                              \
    };                                                                                                               \
                                                                                                                     \
    template <typename T>                                                                                            \
...
struct invoke_table<First> {
  ...
  {
    return &function_trait<First>::template internal_invoker<T, IsInplace>::invoke;
  }
  /// Returns the thunk of an single overloaded callable
  template <typename T>
...
struct invoke_table<First, Second, Args...> {
  using type = std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> const*;
  ...
  template <typename T, bool IsInplace>
  struct invocation_vtable
    : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> {
    constexpr invocation_vtable() noexcept
      : std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>(
            std::make_tuple(
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                                \
  template <                                                                                              \
      ...
      typename Next,                                                                                      \
      typename... Signatures>                                                                             \
  class operator_impl<Index, Function, Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT, Next, Signatures...> \
  ...
  class operator_impl<Index, function<Config, Property>, Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT>    \
  ...
  protected:                                                                                              \
    ...
    Ret operator()(Args... args) CONST VOLATILE OVL_REF NOEXCEPT                                          \
    {                                                                                                     \
      auto parent     = static_cast<function<Config, Property> CONST VOLATILE*>(this);                    \
      using erasure_t = std::decay_t<decltype(parent->erasure_)>;                                         \
                                                                                                          \
      /* `std::decay_t<decltype(parent->erasure_)>` is a workaround for a   */                            \
      /* compiler regression of MSVC 16.3.1, see #29 for details.           */                            \
...
class vtable;
template <bool IsThrowing, bool HasStrongExceptGuarantee, typename... FormalArgs>
class vtable<property<IsThrowing, HasStrongExceptGuarantee, FormalArgs...>> {
  using command_function_t = void (*)(
      vtable* /*this*/,
      opcode /*op*/,
      data_accessor* /*from*/,
  ...
  template <typename T>
  struct trait {
    static_assert(is_box<T>::value, "The trait must be specialized with a box!");
        ...
        std::size_t    to_capacity)
    {
      switch (op) {
        case opcode::op_move: {
          ...
          auto box = static_cast<T*>(retrieve<T>(std::integral_constant<bool, IsInplace>{}, from, from_capacity));
          ...
          // The object is allocated inplace
            ...
            construct(std::true_type{}, std::move(*box), to_table, to, to_capacity);
            box->~T();
          }
          return;
        }
        case opcode::op_copy: {
          auto box = static_cast<T const*>(retrieve<T>(std::integral_constant<bool, IsInplace>{}, from, from_capacity));
          assert(box && "The object must not be over aligned or null!");
...
public:
  ...
  template <typename T>
  static void init(vtable& table, T&& object, data_accessor* to, std::size_t to_capacity)
  {
    trait<std::decay_t<T>>::construct(std::true_type{}, std::forward<T>(object), &table, to, to_capacity);
  }
  ...
  /// Destroys the object at the given position
  void destroy(data_accessor* from, std::size_t from_capacity) noexcept(HasStrongExceptGuarantee)
  {
  ...
  /// vtable
  void weak_destroy(data_accessor* from, std::size_t from_capacity) noexcept(HasStrongExceptGuarantee)
  {
...
struct internal_capacity<Capacity, std::enable_if_t<(Capacity::capacity < sizeof(void*))>> {
  typedef struct {
    /// Tag to access the structure in a type-safe way
    data_accessor accessor_;
  } type;
...
public:
  ...
  FU2_DETAIL_CXX14_CONSTEXPR data_accessor volatile* opaque_ptr() volatile noexcept
  {
    return &storage_.accessor_;
  }
...
public:
  ...
  {
    right.vtable_.copy(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
  }
  ...
  FU2_DETAIL_CXX14_CONSTEXPR erasure(std::false_type /*use_bool_op*/, T&& callable, Allocator&& allocator = Allocator{})
  {
    vtable_t::init(
        vtable_,
  ...
  template <typename T, typename Allocator = std::allocator<std::decay_t<T>>>
  FU2_DETAIL_CXX14_CONSTEXPR erasure(std::true_type /*use_bool_op*/, T&& callable, Allocator&& allocator = Allocator{})
  {
    if (bool(callable)) {
      vtable_t::init(
          vtable_,
  ...
  FU2_DETAIL_CXX14_CONSTEXPR erasure& operator=(std::nullptr_t) noexcept(Property::is_strong_exception_guaranteed)
  {
    vtable_.destroy(this->opaque_ptr(), capacity());
    return *this;
  }

  FU2_DETAIL_CXX14_CONSTEXPR erasure& operator=(erasure&& right) noexcept(Property::is_strong_exception_guaranteed)
  {
    vtable_.weak_destroy(this->opaque_ptr(), capacity());
    right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
    return *this;
  ...
  template <typename T, typename Allocator = std::allocator<std::decay_t<T>>>
  void assign(std::false_type /*use_bool_op*/, T&& callable, Allocator&& allocator = {})
  {
    vtable_.weak_destroy(this->opaque_ptr(), capacity());
    vtable_t::init(
        vtable_,
...
template </*bool IsOwning = false, */ typename Config, bool IsThrowing, bool HasStrongExceptGuarantee, typename... Args>
class erasure<false, Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>> {
  template <bool, typename, typename>
  friend class erasure;
  template <std::size_t, typename, typename...>
  friend class operator_impl;
...
public:
  ...
  constexpr erasure() noexcept
    : invoke_table_(invoke_table_t::template get_empty_invocation_table<IsThrowing>())
    , view_(nullptr)
  {
  }
  ...
  constexpr erasure(std::false_type /*use_bool_op*/, T&& object)
    : invoke_table_(invoke_table_t::template get_invocation_view_table_of<std::decay_t<T>>())
  ...
  constexpr erasure& operator=(std::nullptr_t) noexcept(HasStrongExceptGuarantee)
  {
    invoke_table_ = invoke_table_t::template get_empty_invocation_table<IsThrowing>();
    view_.ptr_    = nullptr;
    return *this;
  ...
  template <typename OtherConfig>
  constexpr erasure& operator=(erasure<true, OtherConfig, property_t> right) noexcept
  {
    invoke_table_ = right.invoke_table_;
    view_         = right.view_;
    return *this;
  ...
  {
    invoke_table_ = invoke_table_t::template get_invocation_view_table_of<std::decay_t<T>>();
    view_.ptr_    = address_taker<std::decay_t<T>>::take(std::forward<T>(callable));
  }
  template <typename T>
...
template <typename T, typename Signature, typename Trait = type_erasure::invocation_table::function_trait<Signature>>
struct accepts_one
  : std::integral_constant<
        bool,
        invocation::can_invoke<typename Trait::template callable<T>, typename Trait::arguments>::value &&
...
struct accepts_all<T, identity<Signatures...>, void_t<std::enable_if_t<accepts_one<T, Signatures>::value>...>>
  : std::true_type {
};
...
template <typename T>
struct has_bool_op<T, void_t<decltype(bool(std::declval<T>()))>> : std::true_type {
#ifndef NDEBUG
  static_assert(!std::is_pointer<T>::value, "Missing deduction for function pointer!");
#endif
};
...
#define FU2_DEFINE_USE_OP_TRAIT(CONST, VOLATILE, NOEXCEPT)                       \
  template <typename Ret, typename... Args>                                      \
  struct use_bool_op<Ret (*CONST VOLATILE)(Args...) NOEXCEPT> : std::true_type { \
  };
...
using enable_if_copyable_correct_t = std::enable_if_t<(!LeftConfig::is_copyable || RightConfig::is_copyable)>;

template <typename LeftConfig, typename RightConfig>
using is_owning_correct = std::integral_constant<bool, (LeftConfig::is_owning == RightConfig::is_owning)>;

/// SFINAES out if the given function2 is not owning correct to this one
template <typename LeftConfig, typename RightConfig>
using enable_if_owning_correct_t = std::enable_if_t<is_owning_correct<LeftConfig, RightConfig>::value>;

template <typename Config, bool IsThrowing, bool HasStrongExceptGuarantee, typename... Args>
class function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>
  : type_erasure::invocation_table::
        operator_impl<0U, function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>, Args...> {
  ...
  using property_t = property<IsThrowing, HasStrongExceptGuarantee, Args...>;
  using erasure_t  = type_erasure::erasure<Config::is_owning, Config, property_t>;

  template <typename T>
  using enable_if_can_accept_all_t = std::enable_if_t<accepts_all<std::decay_t<T>, identity<Args...>>::value>;
  ...
  struct is_convertible_to_this<
      ...
      void_t<enable_if_copyable_correct_t<Config, RightConfig>, enable_if_owning_correct_t<Config, RightConfig>>>
    : std::true_type {
  };

  template <typename T>
  using enable_if_not_convertible_to_this = std::enable_if_t<!is_convertible_to_this<std::decay_t<T>>::value>;

  template <typename T>
  using enable_if_owning_t = std::enable_if_t<std::is_same<T, T>::value && Config::is_owning>;

  template <typename T>
  using assert_wrong_copy_assign_t = typename assert_wrong_copy_assign<Config, std::decay_t<T>>::type;

  template <typename T>
  using assert_no_strong_except_guarantee_t =
      typename assert_no_strong_except_guarantee<HasStrongExceptGuarantee, std::decay_t<T>>::type;
...
public:
  ...
  FU2_DETAIL_CXX14_CONSTEXPR function(function<RightConfig, property_t> const& right)
  ...
  FU2_DETAIL_CXX14_CONSTEXPR function(T&& callable, Allocator&& allocator)
    : erasure_(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable), std::forward<Allocator>(allocator))
  {
  }

  /// Empty constructs the function
  FU2_DETAIL_CXX14_CONSTEXPR function(std::nullptr_t np)
    : erasure_(np)
  {
  }
  ...
  {
    erasure_.assign(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable), std::forward<Allocator>(allocator));
  }
  ...
  friend void swap(function& left, function& right) noexcept(HasStrongExceptGuarantee)
  {
    left.swap(right);
  }

  /// Calls the wrapped callable object
  using type_erasure::invocation_table::operator_impl<0U, function<Config, property_t>, Args...>::operator();
};
...
struct capacity_default : capacity_fixed<detail::object_size::value - (2 * sizeof(void*))> {
};
...
template <typename... Signatures>
using function_view = function_base<
    false,
    true,
    capacity_default,  //
    true,
