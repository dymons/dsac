...
static void Setup(
  ...
  for (int node_id = 0; node_id < N; ++node_id) {
    if (node_id != start.id) [[likely]] {
      ...
      /// узлы
      memo[node_id][(1 << start.id) | (1 << node_id)] =
          graph.GetCost(start, {node_id});
    }
  }
}
    ...
    for (int subset : combinations(r, N)) {
      ...
      for (int next = 0; next < N; ++next) {
        ...
        for (int e = 0; e < N; ++e) {
          ...
          const int new_dist =
              memo[e][subset_without_next] + graph.GetCost({e}, {next});
...
static std::vector<Node> FindOptimalTour(
  ...
  for (int i = N - 1; i >= 1; --i) {
    ...
    for (int j = 0; j < N; ++j) {
      ...
      const int prev_dist =
          memo[index][state] + graph.GetCost({index}, {last_index});
...
TravellingSalesmanResult Solve(CompleteGraph& graph, Node start) {
  ...
  CompleteGraph::AdjacencyMatrix memo(
      N, CompleteGraph::AdjacencyRow(1 << N, 0));
