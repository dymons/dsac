async def test_reset_register_replica_with_unspecified_state(
        registry,
        snapshot,
):
...
async def test_reset_register_replica_with_specified_state(
        registry,
        snapshot,
):
    assert (await registry[8080].post('v1/replica/write', json={'value': 10, 'timestamp': 10})).status_code == 200
    assert (await registry[8081].post('v1/replica/write', json={'value': 10, 'timestamp': 10})).status_code == 200
    assert (await registry[8082].post('v1/replica/write', json={'value': 10, 'timestamp': 10})).status_code == 200
    assert await snapshot() == [
