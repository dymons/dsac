...
Status SentMessage(Descriptor desc, MessageView msg) {
  std::cout << fmt::format("Sent message \"{}\" to desc {}", msg, desc)
            << std::endl;
...
Response ReceiveMessage(Descriptor desc) {
  ...
  if (bytes < 0) {
    ...
    msg.resize(bytes);
    std::cout << fmt::format("Receive msg \"{}\" from desc {}", msg, desc)
              << std::endl;
  }
...
int main() {
  ...
  int const error =
      connect(remote_desc, (sockaddr*)(&socket_desc), sizeof(socket_desc));
  if (error < 0) {
  ...
  while (true) {
    ...
    std::string message;
    std::cin >> message;
    if (Status status = SentMessage(remote_desc, message);
        status == Status::Error) {
      break;
    }

    if (Response res = ReceiveMessage(remote_desc);
