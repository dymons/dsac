...
TEST_CASE("Testcases are sorted using bubble sort algorithm (D)", "[bubble_sort][default]")
{
  using testcase = std::vector<int>;
  SECTION("Sorting an empty testcase")
  {
    auto empty_testcase = testcase{};
    dsac::bubble_sort(std::begin(empty_testcase), std::end(empty_testcase));
  ...
  SECTION("Sorting a sorted testcase")
  {
    ...
    {
      dsac::bubble_sort(std::begin(sorted_testcase), std::end(sorted_testcase));
      REQUIRE(dsac::is_sorted(sorted_testcase));

      dsac::bubble_sort(sorted_testcase);
      REQUIRE(dsac::is_sorted(sorted_testcase));
    }

    SECTION("Using custom predicate")
    {
      dsac::bubble_sort(sorted_testcase, std::greater<int>{});
      REQUIRE(dsac::is_sorted(sorted_testcase, std::greater<int>{}));
    }
  }
}

TEST_CASE("Testcases are sorted using bubble sort algorithm (S)", "[bubble_sort][stress]")
{
  ...
  for (std::size_t i{}; i < kNumberOfIteration; ++i) {
    ...
    dsac::bubble_sort(std::begin(testcase), std::end(testcase));
